...
import { getApod, ApodData } from "../services/apodService";
...
const formatDayOfWeek = (dateString: string): string => {
  const date = new Date(dateString + "T00:00:00"); // Ensure correct parsing by adding time
...
const DashboardScreen = () => {
  ...
  const fetchDashboardData = useCallback(async (forceRefreshApod = false) => {
    ...
    setApodError(null);
    try {
      const data = await getApod(forceRefreshApod);
      setApodData(data);
      if (!data && !forceRefreshApod) { // Show error only if not a forced refresh and no data
        setApodError(
    ...
    const now = new Date();
    setCurrentDayInfo(
      `${formatDayOfWeek(now.toISOString().split("T")[0])}, ${formatFullDate(now.toISOString().split("T")[0])}`
    );

    // Load upcoming events (Next 2)
    try {
      const today = new Date().toISOString().split("T")[0];
      const sortedEvents = (astronomicalEventsData as { id: string; day: number; month: number; year: number; title: string; description: string; color: string; viewing_tips: string[]; }[])
        .map(event => ({
          id: event.id,
          date: `${event.year}-${String(event.month).padStart(2, '0')}-${String(event.day).padStart(2, '0')}`, // Formato YYYY-MM-DD
          title: event.title,
          description: event.description,
          type: event.color, // Ou outra lógica para definir o tipo
    ...
    // Load random meditations (2)
    try {
      const allMeditations = meditationJsonData as Meditation[];
      const nonPremiumMeditations = allMeditations.filter(m => !m.is_premium);
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    await fetchDashboardData(true); // Force refresh APOD
    setRefreshing(false);
  }, [fetchDashboardData]);
  ...
  const renderApodSection = () => {
    ...
    if (apodError && !apodData) { // Show error only if there's no data to display
      return (
      ...
      return (
        ...
        >
          ...
          {apodData.media_type === "image" ? (
            <Image
              source={{ uri: apodData.hdurl || apodData.url }} // Prefer HD URL
    ...
    return (
        <View style={styles.apodContainer}>
          <Text style={styles.errorText}>Nenhuma imagem astronômica disponível no momento. Tente atualizar.</Text>
        </View>
    ); // Fallback if no data and no error (e.g. after a failed refresh)
  };
...
const styles = StyleSheet.create({
  ...
  apodContainer: {
    ...
    borderRadius: 20,
    padding: 20,
    minHeight: 150, // Ensure a minimum height for loading/error states
    justifyContent: "center", // Center content for error/loading states
  },
  apodTitle: {
  ...
  skeletonContainer: {
    // padding: 20, // Already on apodContainer
  },
  skeletonImage: {
    width: "100%",
    backgroundColor: COLORS.backgroundTertiary,
