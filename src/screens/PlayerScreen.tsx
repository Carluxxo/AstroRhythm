...
import { RootStackParamList, PlayerScreenParams } from "../navigation/types"; // Ensure PlayerScreenParams is defined in types.ts
...
const PlayerScreen = () => {
  ...
  const {
    ...
    sound: contextSound, // Direct access to sound object if needed for advanced status, but prefer context values
    positionMillis,
    durationMillis,
    playTrack,
    pauseTrack,
  ...
  const params = route.params as PlayerScreenParams; // Type assertion for route params
  ...
  const trackToPlay: Track | null = params ? {
    ...
    // duration will be derived from sound object via context
  } : null;

  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [isSeeking, setIsSeeking] = useState(false);
  const [sliderValue, setSliderValue] = useState(0); // Slider value from 0 to 1

  // Effect to play the track when the screen is focused with new track params
  useEffect(() => {
    if (trackToPlay) {
      if (contextTrack?.id !== trackToPlay.id || contextTrack?.audio_url !== trackToPlay.audio_url) {
        ...
        // Track is already the one in context, ensure loading is false
        setIsLoading(false);
      }
    }
  }, [trackToPlay?.id, trackToPlay?.audio_url, playTrack, contextTrack]);

  // Update slider position based on context's positionMillis and durationMillis
  useEffect(() => {
  ...
  const handlePlayPause = () => {
    ...
    if (isPlaying) {
      ...
      // If not playing, and it's the current track, resume. Otherwise, play (which handles loading).
      if (contextTrack?.id === trackToPlay.id) {
        ...
        // This case should ideally be handled by the useEffect above that calls playTrack
        // but as a fallback or if contextTrack is null but we have trackToPlay:
        setIsLoading(true);
        playTrack(trackToPlay)
            .then(() => setIsLoading(false))
            .catch(err => {
  ...
  const handleSeekEnd = async (value: number) => {
    ...
    // Delay setting isSeeking to false to allow context to update positionMillis first
    // This helps prevent the slider from jumping back momentarily
  ...
  const handleToggleFavorite = () => {
    if (contextTrack) { // Only allow favoriting if a track is loaded in context
    ...
    <View style={styles.container}>
      ...
      <View style={styles.visualizationContainer}>
        <View style={styles.visualization}>
          {currentArtwork ? (
            <Image source={{ uri: currentArtwork }} style={styles.visualizationImage} resizeMode="cover" />
          ) : (
      ...
      <View style={styles.progressContainer}>
        <Slider
          ...
          onValueChange={handleSeekChange} // Continuously update sliderValue state
          onSlidingComplete={handleSeekEnd} // Call seekTrack on completion
...
const styles = StyleSheet.create({
  ...
  playIconOffset: {
    // marginLeft: 3, // Minor adjustment if play icon is not centered visually
  },
  errorText: {
