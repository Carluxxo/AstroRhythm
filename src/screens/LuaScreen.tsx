...
import { View, Text, StyleSheet, ScrollView, Image, Platform, Pressable, FlatList, TouchableOpacity } from 'react-native';
...
const COLORS = {
  ...
  textTertiary: '#A0A0B0',
  accentSecondary: '#4ECDC4', // Added from guidelines
};
...
const FONTS = {
  ...
  bodyMedium: 'Inter-Medium', // Added from guidelines
};
...
// EDITED: CalendarDay can now be null for empty cells
interface CalendarDay {
  date: Date;
  dayOfMonth: number;
  isCurrentMonth: boolean; // This will always be true for non-null items now
  phaseIcon: string;
}
...
const LuaScreen = () => {
  ...
  const findPhaseStartDate = useCallback((targetDate: Date): Date => {
    ...
    for (let i = 0; i < 8; i++) { // A phase is typically not longer than 4-5 days, 8 is safe
      ...
      if (tempPhaseName === targetPhaseName) {
        ...
        break; // Phase changed, we found the start date on the previous iteration
      }
    }
    return startDate;
  ...
  useEffect(() => {
    ...
    const scheduleNextUpdate = () => {
      const now = new Date();
      const endOfDay = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 0, 0, 1, 0); // 1 second past midnight
    ...
    updateMoonData(); // Initial update
    scheduleNextUpdate(); // Schedule the first midnight update
  ...
  const handleMoonPress = (moonData: MoonPhase, date: Date | null, customMessage?: string) => {
    ...
    // If the clicked phase is the same as the current actual phase, display "Current Phase".
    ...
    // Otherwise, use the existing logic for future/past dates
    let targetDate = date;
  ...
  const handleDayPress = (day: CalendarDay | null) => {
    ...
    // For future days, find the start date of that phase for the countdown
                  ...
                  return (
                    ...
                    >
                      <View style={[styles.dayCell, isToday && styles.todayCell]}>
                        <Image source={moonImages[item.phaseIcon]} style={styles.dayMoonIcon} />
...
const styles = StyleSheet.create({
  ...
  mainMoonImage: { width: 120, height: 120, resizeMode: 'contain', zIndex: 1 },
  moonGlow: {
    width: 115, // Slightly smaller than the moon image
    height: 115, // Slightly smaller than the moon image
    borderRadius: 115 / 2,
    backgroundColor: 'transparent', // No solid color
    position: 'absolute',
    top: 22.5, // Center the smaller glow view
    shadowColor: COLORS.accentPrimary,
    shadowOffset: { width: 0, height: 0 },
    shadowOpacity: 0.9, // Make the glow intense
    shadowRadius: 18, // Blur radius for the glow effect
    zIndex: 0,
  },
  ...
  carouselMoonImage: { width: 60, height: 60, resizeMode: 'contain', marginBottom: 12 },
